...
function getPerAttribute(typeset) {
	
	//Make sure valid inputs are given
	var entry = Number(document.getElementById(typeset+"Number").value)
	if(document.getElementById(typeset+"AttributeList").value == "none"){ alert("No input given!"); return; }
	if( isNaN(entry) || entry <= 0){ alert("Invalid value given!"); return;}
	
	var attrHolder = document.createElement("p");
	var selected = document.getElementById(typeset+"AttributeList").value;
	
	selected = selected.split(","); //We want the stored_at, state, and phyVal all seperated
	
	var techID = selected[3];
